...
class ExaminationService implements IService
{
    ...
    public function dissociateStudyArea(int $examinationId, int $studyAreaId): ServiceResponse
    {
        \Log::info("Iniciando desassociação: Examination ID = $examinationId, Study Area ID = $studyAreaId");
        ...
        try {
            ...
            if (!$examination->studyAreas()->where('study_area_id', $studyAreaId)->exists()) {
                \Log::info("Associação não existe: Examination ID = $examinationId, Study Area ID = $studyAreaId");
                $this->serviceResponse->setAttributes(409, (object)[
                    'message' => 'Associação não existente.',
                    'examination_id' => $examinationId,
                    'study_area_id' => $studyAreaId,
                ]);
                return $this->serviceResponse;
            ...
            $examination->studyAreas()->detach($studyAreaId);
            \Log::info("Desassociação realizada com sucesso: Examination ID = $examinationId, Study Area ID = $studyAreaId");
